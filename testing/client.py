...
try:
    ...
    for msg_id, msg in rms_messages.items():
        ...
        for signal in msg.signals:
            logger.info(f"  └─ {signal.name}: {signal.comment or ''} [{signal.unit or 'no unit'}]")
...
def get_message_info(message_id):
    ...
    if message_id in rms_messages:
        ...
        return {
            ...
            'signals': {sig.name: {'unit': sig.unit, 'comment': sig.comment} for sig in msg.signals}
        }
    ...
    try:
        ...
        try:
            if msg_info['source'] == 'RMS(motor controller)':
                decoded_values = dbc_rms.decode_message(message.arbitration_id, message.data)
            else:
...
def format_can_message(message):
    ...
    if decoded_message:
        return {
            ...
            'signals': decoded_message['signals'],  # Now contains both definitions and values
